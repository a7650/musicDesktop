  ...
  <div class="rank">
    ...
    <div class="rank-content">
      <ul class="items" ref="items">
        <li v-for="(item,index) in topList" :key="item.id" :class="{'active':active==index}" class="item"  @click="selectList(item,index)"> 
          <div class="bg">
    ...
    _getRankList() {
      getRankList().then(
        data => {
          ...
          this.pageNum = Math.ceil(data.topList.length/5);
        },
        err => {
          console.log(err);
        }
